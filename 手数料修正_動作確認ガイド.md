...
- [ ] **Tier（ランク）情報が表示される**
  - 現在の Tier 番号とランク名（例: Tier 1 ビギナー）
  - 現在の手数料率（例: 4.5%）
  - 今月の QR 決済回数
  - 次のランクへの条件（例: あと 3 回で Tier 2 になり、手数料が 4.2%になります）
...
```json
{
  ...
  "data": {
    ...
    "tierDefinitions": {
      "1": { "name": "ビギナー", "min": 0, "max": 3, "defaultRate": 0.045 },
      "2": { "name": "レギュラー", "min": 4, "max": 10, "defaultRate": 0.042 }
    }
  }
}
```
