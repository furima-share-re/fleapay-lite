...
- 各プラン（standard, pro, kids）に対して有効な手数料率が 1 件ずつ存在
...
2. Stripe Checkout Session の`application_fee_amount`が設定されているか
...
- `ENABLE_STRATEGY_F_TIER_SYSTEM=false`: 従来の plan_type ベース
- `ENABLE_STRATEGY_F_TIER_SYSTEM=true`（デフォルト）: Tier 制対応
...
**URL**: `/seller-dashboard?s=test-seller-pro`

**確認ポイント**:

- [ ] **Tier（ランク）情報が表示される**
  - 現在の Tier 番号とランク名（例: Tier 1 村）
  - 現在の手数料率（例: 4.8%）
  - 今月の QR 決済回数
  - 次のランクへの条件（例: あと 3 回で Tier 2 になり、手数料が 4.4%になります）
...
```json
{
  ...
  "data": {
    "tier": {
      "number": 1,
      "name": "村",
      "currentFeeRate": 0.048,
      "currentFeeRatePercent": "4.80",
      "nextFeeRate": "4.40",
      "feeRateMessage": "あと3回のQR決済でTier 2（町）になり、手数料が4.4%になります！"
    },
    "transactionCount": {
      "current": 1,
      "range": {
...
- [ ] **手数料率マスタが表示される**
  ...
  - Tier 別の手数料率（Tier 1-5）
  - 有効期間（effective_from, effective_to）
- [ ] **Tier 定義が表示される**
  - 各 Tier の名前、QR 決済回数範囲、手数料率
...
```json
{
  ...
  "data": {
    ...
    "tierDefinitions": {
      "1": { "name": "村", "min": 0, "max": 3, "defaultRate": 0.048 },
      "2": { "name": "町", "min": 4, "max": 10, "defaultRate": 0.044 }
    }
  }
}
```
...
#### 4.2 Stripe ダッシュボードでの確認

**確認項目**:

1. **Payment Intent**:

   - `application_fee_amount`: 手数料額（例: 70 円）
   - `transfer_data.destination`: 出店者の Stripe アカウント ID
   - `amount`: 注文金額（例: 1000 円）
...
- `amount_gross`: 1000 円（顧客から請求された金額）
- `amount_net`: 930 円（出店者への送金額）
- `amount_fee`: 70 円（手数料）
...
- Standard プラン: `test-seller-standard`
- Pro プラン: `test-seller-pro`
- Kids プラン: `test-seller-kids`

**確認手順**:

1. 各プランで 1000 円の注文を作成
2. チェックアウト API を呼び出し
3. ログで手数料率と手数料額を確認
...
- Tier 1: 0-10 回 → 手数料率: 設定値
- Tier 2: 11-50 回 → 手数料率: 設定値
- Tier 3: 51-200 回 → 手数料率: 設定値
- Tier 4: 201-1000 回 → 手数料率: 設定値
- Tier 5: 1001 回以上 → 手数料率: コミュニティ目標達成状況に応じて変動
...
1. **出店者ダッシュボード**: `http://localhost:3000/seller-dashboard?s=test-seller-pro`

   - **Tier（ランク）情報の確認**:
     - 「現在のランク」カードが表示されているか確認
     - Tier 番号、ランク名、手数料率が表示されているか確認
     - 今月の QR 決済回数が表示されているか確認
...
2. チェックアウト画面で決済
3. Stripe テストカードで決済完了
4. Stripe ダッシュボードで手数料を確認

---

## ⚠️ 注意事項

1. **Stripe Connect 必須**: 出店者は Stripe Connect の接続アカウントが必要
