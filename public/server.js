  ...
  try {
    ...
    if (t === "payment_intent.succeeded") {
      ...
      if (!sellerId) {
        ...
        // 【修正v2】stripe_paymentsテーブルに記録 - UNIQUE制約なし対応
        // 既存レコードをチェック
        const existing = await pool.query(
          `select id from stripe_payments where payment_intent_id=$1 limit 1`,
          [pi.id]
        );
        ...
        if (existing.rows.length > 0) {
          ...
          await pool.query(
            `update stripe_payments set
              charge_id = $1,
              balance_tx_id = coalesce($2, balance_tx_id),
              amount_gross = $3,
              amount_fee = coalesce($4, amount_fee),
              amount_net = $5,
              status = $6,
              raw_event = $7,
              updated_at = now()
            ...
            [
              chargeId,
              null,          // balance_tx_id (後続イベントで更新)
              amount,        // amount_gross
              null,          // amount_fee (後続イベントで更新)
              amount,        // amount_net (初期値)
              "succeeded",   // status
              event,         // raw_event
              pi.id          // payment_intent_id (WHERE条件)
            ]
          );
        } else {
          // 挿入
          await pool.query(
            ...
            [
              ...
              chargeId,      // $4
              null,          // $5: balance_tx_id (後続イベントで更新)
              amount,        // $6: amount_gross
              null,          // $7: amount_fee (後続イベントで更新)
              amount,        // $8: amount_net (初期値)
              currency,      // $9
              "succeeded",   // $10: status
