...
| **Phase 3.3** | Langfuse + Promptfoo導入 | Week 5 Day 1-2 | ⏳ 未着手 | **本番環境DB移行後**（LLM観測・評価ツール導入、実装工数: 2.5-4時間） |
...
### Phase 3.3: Langfuse + Promptfoo導入

**期間**: Week 5 Day 1-2  
**優先度**: 🟢 **中**  
**目的**: LLM観測・評価ツール導入による品質向上

**実施内容**:
1. **Langfuse導入（LLM観測・トレーシング）**
...
**完了条件**:
- ✅ LangfuseダッシュボードでLLM API呼び出しのトレーシング確認
- ✅ プロンプト・レスポンスの詳細分析が可能
- ✅ Promptfooでプロンプトテスト・評価が実行可能
- ✅ CI/CDで自動的にプロンプト品質チェックが実行される

**期待効果**:
- LLM API呼び出しの詳細な観測・分析が可能
- プロンプト品質の継続的な改善サイクル構築
- プロンプト変更時の回帰テスト自動化

**注意事項**:
- Heliconeと併用可能（Helicone: プロキシ経由の自動記録、Langfuse: SDK統合の詳細分析）
- 実装順序による工数の増減はない（Helicone実装後でも工数は変わらない）
- 本番環境DB移行後に実施（Phase 1.8完了後）

**実装工数合計**: **2.5-4時間**（コーディング）+ **1時間10分-2時間15分**（事前準備・確認）
