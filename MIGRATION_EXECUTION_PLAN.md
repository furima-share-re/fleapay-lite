...
**プロジェクト**: fleapay-lite  
**作成日**: 2025-01-15  
**最終更新**: 2026-01-04（Phase 1.8完了、本番環境DB移行完了）  
...
| **Phase 3.1** | Helicone導入（LLM API監視） | 2026-01-03 | ✅ **完了** | Helicone導入完了（検証環境・本番環境）、動作確認完了、ダッシュボードでリクエスト可視化確認済み |
| **Phase 1.8** | 本番環境DB移行 | 2026-01-04 | ✅ **完了** | 本番環境データベース移行完了（GitHub Actions経由でSQLファイルインポート成功）、Supabaseへの移行完了 |

### 未実装フェーズ（優先順位順）

| Phase | フェーズ名 | 予定期間 | 状態 | 備考 |
|-------|----------|---------|------|------|
| **Phase 3.2** | Sentry導入（エラー監視） | Month 3-4 | ⏳ **未着手** | **本番環境のみ導入**（検証環境には不要） |
| **Phase 3.3** | Cloudflare WAF設定 | Month 3-4 | ⏳ **未着手** | **本番環境のみ導入**（検証環境には不要） |
| **Phase 3.4** | 詳細ヘルスチェック + 自動ロールバック | Month 3-4 | ⏳ **未着手** | **中優先度**（運用自動化） |
| **Phase 1.7** | RLS実装 | Week 4 Day 1-2 | ⏳ 未着手 | **最終工程**（Phase 1.8完了後） |
| **Phase 3.5** | Langfuse + Promptfoo導入 | Week 5 Day 1-2 | ⏳ 未着手 | **本番環境DB移行後**（LLM観測・評価ツール導入、実装工数: 2.5-4時間） |
...
- **Phase 3.1（Helicone導入）は完了**（Helicone: 検証環境・本番環境導入完了、動作確認完了、ダッシュボードでリクエスト可視化確認済み）
- **Phase 3.2（Sentry導入）とPhase 3.3（Cloudflare WAF設定）を次に実施**（本番環境のみ、検証環境には不要）
- **Phase 3.4（詳細ヘルスチェック + 自動ロールバック）をその後実施**（運用95%自動化達成のため）
- **Phase 1.8（本番環境DB移行）は完了**（GitHub Actions経由でSQLファイルインポート成功、Supabaseへの移行完了）
- **Phase 1.7（RLS実装）は最終工程**（Phase 1.8完了後、本番環境に適用）
